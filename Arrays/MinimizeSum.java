...
You only need to complete the function Maximize() that takes an array (arr), sizeOfArray (n), and return the maximum value of an array. 
...
class MinimizeSum{
	//O(nlog(n))
	static long maximize(long[] arr, int n){
		long sum = 0;
		...
		return sum % 1000000009; // Since output could be large, hence module 109+7 
 	}
